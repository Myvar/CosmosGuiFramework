...
namespace SharpFont {
    public interface IGlyphAtlas {
        ...
        void Insert (int page, int x, int y, int width, int height, byte[] data);
    }
            ...
            while (text != end) {
                ...
                if (!cachedFace.Glyphs.TryGetValue(codePoint, out glyph) && !char.IsControl(c)) {
                    ...
                    if (width > 0 && height > 0) {
                        ...
                        atlas.Insert(currentPage, rect.X, rect.Y, rect.Width, rect.Height, mem.memory);
                    }
